...
static void sTireCalc (UserPars *mp, int *job, const char *suffix) {
  ...
  case 1:
    ...
    sMessage("*** Made by Yukio Watanabe - December 9, 2015   ***");
    sMessage("*************************************************** \n");
  ...
  case 2:{
    ...
    sMessage(str);
    sprintf(str, "%.1f: Aligning stiffness (N-m/rad) \n", mp->typarr[5]);
    sMessage(str);
...
static void sCalcTireCtcForces(int id, vs_real w) {
  ...
  if (mp->fz < 0.01) {
    ...
    vs_real abs_vx = max(0.01, fabs(mp->vx));
    mp->kappa = (mp->r_free*w - (mp->vx)) / abs_vx;
    mp->alpha = atan(mp->vy / abs_vx);

    if (mp->mu > 0.0) {
      mp->fx = mp->cfx * mp->kappa * mp->mu;
      mp->fy = mp->cfy * mp->alpha * mp->mu;
      mp->mz = mp->cmz * mp->alpha * mp->mu;
      mp->mx = mp->cmx * mp->gamma * mp->mu;
    }
    else {
      mp->fx = mp->cfx * mp->kappa;
      mp->fy = mp->cfy * mp->alpha;
