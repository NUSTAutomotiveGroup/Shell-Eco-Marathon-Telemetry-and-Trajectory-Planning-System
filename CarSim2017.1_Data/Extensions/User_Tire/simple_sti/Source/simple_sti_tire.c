...
void simple_sti(
  ...
  pitch = -asin(tramat[2]);
  roll = atan(tramat[5] / tramat[8]);
  yaw = atan(tramat[1] / tramat[0]);
  ...
  case 1:
    ...
    message("*** Made by Yukio Watanabe - December 9, 2015   ***");
    message("*************************************************** \n");
...
static void sCalcTireCtcForces(int id, vs_real w) {
  ...
  if (var->fz < 0.01) {
    ...
    vs_real abs_vx = max(0.01, fabs(var->vx));
    var->kappa = (var->r_free*w - (var->vx)) / abs_vx;
    var->alpha = atan(var->vy / abs_vx);

    if (var->mu > 0.0) {
      var->fx = var->cfx * var->kappa * var->mu;
      var->fy = var->cfy * var->alpha * var->mu;
      var->mz = var->cmz * var->alpha * var->mu;
      var->mx = var->cmx * var->gamma * var->mu;
    }
    else {
      var->fx = var->cfx * var->kappa;
      var->fy = var->cfy * var->alpha;
